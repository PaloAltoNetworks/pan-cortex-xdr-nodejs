 ...
 */
async function policyEndpoint(endpointId) {
    const payload = {
        request_data: { endpoint_id: endpointId }
    };
    const { data, status } = await this.session.post(api_endpoint_1.policyEndpointPath, api_endpoint_1.endpointApiErrBldr, payload);
    if (typeof data == 'string')
        throw new xdrerror_1.XdrError('endpoint', data, status);
    return data;
}
exports.policyEndpoint = policyEndpoint;
