 ...
 */
async function blacklistFile(hashList, comment) {
    const payload = {
        request_data: {
            hash_list: hashList,
            comment
    ...
    const { data, status } = await this.session.post(api_hash_exceptions_1.blackListPath, api_hash_exceptions_1.hashExceptionApiErrBldr, payload);
    if (typeof data == 'string')
        throw new xdrerror_1.XdrError('endpoint', data, status);
    return data;
}
exports.blacklistFile = blacklistFile;
