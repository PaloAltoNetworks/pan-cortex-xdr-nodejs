 ...
 */
async function managementLog(filters, search_from, search_to, sort) {
    const payload = {
        request_data: {
            filters,
            search_from,
    ...
    const { data, status } = await this.session.post(api_audits_1.mgmtLogsPath, api_audits_1.auditApiErrBldr, payload);
    if (typeof data == 'string')
        throw new xdrerror_1.XdrError('alerts', data, status);
    return data;
}
exports.managementLog = managementLog;
