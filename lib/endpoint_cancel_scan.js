 ...
 * once.
 */
async function cancelScanEndpoint(filters) {
    const payload = {
        request_data: { filters }
    };
    const { data, status } = await this.session.post(api_endpoint_1.cancelScanEndpointPath, api_endpoint_1.endpointApiErrBldr, payload);
    if (typeof data == 'string')
        throw new xdrerror_1.XdrError('endpoint', data, status);
    return data;
}
exports.cancelScanEndpoint = cancelScanEndpoint;
